...
def train_vqvae(model, training_data, n_epochs, lr, batch_size, device):
    ...
    for i in range(n_epochs):
        for k, (feat_x, feat_y) in enumerate(batched(training_data, batch_size)):
            feat_x  = torch.tensor(feat_x, dtype=torch.float32).to(device)
            feat_y = torch.tensor(feat_y, dtype=torch.float32).to(device)
...
if __name__ == '__main__':
    # Start
    seed = 0
    data_path = 'tmp/vaevq_training_data.npy'
    out_dir = 'tmp'
    n_states = 20  # alphabet size 128 512
    ...
    np.savetxt(f'{path}/states{name}.txt', model.vq.embedding.weight.detach().cpu().numpy())